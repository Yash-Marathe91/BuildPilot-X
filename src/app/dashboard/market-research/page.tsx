...
import { Search, Sparkles, Briefcase, Megaphone, Target, BarChart3 } from "lucide-react";
...
const AGENTS = [
  ...
  { id: "cpo", name: "Aura", role: "CPO", icon: Search, color: "text-emerald-400", border: "border-emerald-400/30" },
];

export default function MarketResearchPage() {
  const [isGenerating, setIsGenerating] = useState(false);
  const [isComplete, setIsComplete] = useState(false);
  const [agentResponses, setAgentResponses] = useState<Record<string, string>>({ cmo: "", cpo: "" });
    ...
    try {
      await AIClient.streamCompetitors(
        { idea: "A high-performance enterprise SaaS platform requiring real-time multi-agent orchestration." },
        {
          ...
          onError: (error) => {
            console.error(error);
            toast.error("Failed to stream market research.");
    ...
    <div className="flex flex-col gap-8 pb-12 h-full">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-4xl font-bold tracking-tight">Market Research</h1>
          <p className="text-muted-foreground text-lg mt-1">Analyze target demographics, market size, and customer validation signals.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button className="rounded-xl h-11 px-6 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold shadow-[0_0_15px_rgba(16,185,129,0.3)]">
